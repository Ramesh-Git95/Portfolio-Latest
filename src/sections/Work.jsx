  ...
  return (
    <section>
        ...
        <div className='flex items-center justify-center gap-8 cursor-pointer'>
          <span className={`bg-slate-900 p-2 rounded-lg text-white hover:bg-slate-700 ${filter === '#ReactJs' ? 'bg-blue-500' : ''}`}
                onClick={() => setFilter('#ReactJs')}> React Js
        </span>
                    <span
                        className={`bg-slate-900 p-2 rounded-lg text-white hover:bg-slate-700 ${filter === '#HTML/CSS' ? 'bg-blue-500' : ''}`}
                        onClick={() => setFilter('#HTML/CSS')}> HTML/CSS</span>
                    <span
                        className={`bg-slate-900 p-2 rounded-lg text-white hover:bg-slate-700 ${filter === '#UI/UX' ? 'bg-blue-500' : ''}`}
                        onClick={() => setFilter('#UI/UX')}> UI/UX</span>
                </div>
                        ...
                        >
                            <div className="px-6 py-6 bg-slate-950">
                                <div className='flex justify-center items-center'>
                                    <img className="h-48 w-48 rounded-lg mb-3 flex justify-center items-center" src={project.image}
                                         alt={project.title}/>
                                </div>
                            ...
                            <div className="px-4 pt-4 pb-4 bg-slate-950 rounded-b-lg flex">
                                {project.tags.map((tag, index) => (
                                    <span key={index}
                                          className="inline-block bg-gray-400 rounded-sm px-1 py-1 text-sm font-bold text-black mr-2 mb-2">{tag}</span>
                                ))}
                            </div>
                        </motion.div>
                    ))}
